...
const sendMatchedAvoidablesResponse = function(matchedAvoidables){
...
const handleProductAPIResponse = function(req, res) {
  ...
  return function(err, product) {
    ...
    User.getMatchedAvoidables(user_id, product.ingredients, sendMatchedAvoidablesResponse);
  }
}
...
requestHandler.addFavorites = function(req, res){
  ...
  User.findOneAndUpdate({user_id: req.body.user_id}, {$push: {favorites: newProd._id }}, function(err, prod){
    if (err) {
