...
const handleProductAPIResponse = function(req, res) {
  ...
  return function(err, product) {
    ...
    Product.create(product, function(err, newProduct){
       User.getMatchedAvoidables(req.query.user_id, newProduct.ingredients, sendMatchedAvoidablesResponse(req, res, newProduct));
    });
  }
}
...
requestHandler.getFavorites = function(req, res){
   if (!req.query.user_id) {
    res.status(403).json({error: 'no user_id given\n'});
    return;
  }
...
requestHandler.addFavorites = function(req, res) {
    if (!req.body.product_id) {
      res.status(403).json({error: 'no product_id given'});
      return;
    }
...
requestHandler.deleteShoppingListItem = function(req, res){
  if (!req.body.product_id) {
    res.status(403).json({error: 'no product_id given'});
    return;
  }
...
requestHandler.deleteShoppingList = function(req, res){
